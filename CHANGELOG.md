...
### Исправлено
- **Смена пароля admin** ([#27](https://github.com/ildarmaga/wdtt/issues/27)): логин не декодировал `application/x-www-form-urlencoded` (`P@ss` → `P%40ss`), поэтому после смены пароля со спецсимволами вход ломался. Теперь как при сохранении настроек — через `url.ParseQuery`.

## [1.4.72] — 2026-08-03

### Изменено
- **Мобильная панель**: «Пользователи» и «Подключения» на телефоне — карточки без горизонтального скролла; меню WDTT только «Изменить» (ссылка/рестарт/юзеры — на главной).
...
### Исправлено
- **VK_HASH не сохранялся** ([#23](https://github.com/ildarmaga/wdtt/issues/23)): reconcile затирал ручные хеши, если не было живых `vk_calls`. Теперь снимаются только finishing creator-хеши; ручные остаются. Пустой хеш в ссылке давал литерал `VK_HASH`.

## [1.4.69] — 2026-08-03

### Исправлено
- **VK_HASH**: колонка в «Пользователи» синхронизируется с живыми `vk_calls` (мёртвые хеши с профиля снимаются).
...
- **Лимит скорости**: HTB больше не блокируется qdisc `fq` (BBR) на `wdtt0`; после Save лимит сразу применяется; дробные Мбит/с (0.1); live смена через `tc class replace` без reconnect.
- **VK Creator**: «Завершить» показывает «завершается»; live-проверка cookies (устарели); удаление пользователя останавливает VK-сессии и чистит `vk_calls`; убраны инструкция Chrome/remixsid и блок входа по паролю из панели.
